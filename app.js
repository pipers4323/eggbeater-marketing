...
const STORE = {
  ...
  SCORER_SESSIONS:  'ebwp-scorer-sessions',
  SCORER_CONFLICTS: 'ebwp-scorer-conflicts',
  LIVE_ACTIVITY_GAME:'ebwp-live-activity-game',
...
function _meaningfulEventCount(score) {
  return Array.isArray(score?.events) ? score.events.filter(ev => ev && ev.type !== 'game_state').length : 0;
}
...
function _attachScoreContext(score, gameOrRef, explicitGroupKey = '') {
  ...
  const tournament = getTournamentForGroup(_contextGroupKey(gameOrRef, explicitGroupKey)) || TOURNAMENT || {};
  if (game) {
    if (!score.opponent && game.opponent) score.opponent = game.opponent;
    if (!score.time && game.time) score.time = game.time;
    if (!score.dateISO && game.dateISO) score.dateISO = game.dateISO;
    if (!score.date && game.date) score.date = game.date;
    if (!score.location && game.location) score.location = game.location;
  }
  if (!score.gameId) score.gameId = _gameIdOnly(gameOrRef);
  if (!score.tournamentId && tournament?.id) score.tournamentId = tournament.id;
  return score;
}

function _normalizeSlotValue(val) {
...
function _isActiveLocalScorerScore(localScore, gameOrRef, explicitGroupKey = '') {
  ...
  return _hasActiveScorerSession(gameOrRef, explicitGroupKey) || myGames.has(scopedKey) || myGames.has(rawKey);
}
...
function _shouldPreserveLocalLiveScore(localScore, remoteScore, gameOrRef = null, explicitGroupKey = '') {
  ...
  return localScore.gameState !== 'final';
}
...
function _adoptLocalDraftForOfficialGame(game, explicitGroupKey = '') {
  ...
  const match = _findMatchingLocalDraftForGame(game, explicitGroupKey);
  if (!match) return existing || null;
  const adopted = { ...match.score };
  _attachScoreContext(adopted, game, explicitGroupKey);
  state.liveScores[scopedKey] = adopted;
  if (match.key !== scopedKey) {
    _recordScorerConflict('draft-adopted', {
      scopedKey,
      groupKey: explicitGroupKey || game?._groupKey || '',
      gameId: game?.id || '',
      officialGameId: game?.id || '',
      localKey: match.key,
      opponent: game?.opponent || adopted.opponent || '',
      time: game?.time || adopted.time || '',
      note: 'Local draft was rebound onto an official game record.',
    });
    delete state.liveScores[match.key];
  }
  return adopted;
}
...
function _hydrateOfficialResultsFromTournament() {
  ...
  const applyGames = (games, explicitGroupKey = '') => {
    ...
    games.forEach(g => {
      ...
      _attachScoreContext(nextScore, g, explicitGroupKey || g._groupKey || '');
  ...
  const mergedGames = tournament.games.map((g) => {
    ...
    if (_shouldPreserveLocalLiveScore(preservedLocal, scored.score, g, explicitGroupKey || g._groupKey || '')) {
      return {
        ...
        liveScore: preservedLocal,
        teamScore: typeof preservedLocal.team === 'number' ? preservedLocal.team : (g.teamScore ?? g.liveScore?.team),
        oppScore: typeof preservedLocal.opp === 'number' ? preservedLocal.opp : (g.oppScore ?? g.liveScore?.opp),
        result: g.result || '',
      };
    }
    const merged = {
    ...
    _attachScoreContext(merged.liveScore, merged, explicitGroupKey || g._groupKey || '');
    return merged;
  });
...
function _mergeScoredGamesIntoLocalState(scoredGames, explicitGroupKey = '') {
  ...
  for (const [gameId, scored] of Object.entries(scoredGames)) {
    ...
    const currentGame = _findGameByRef(gameId, explicitGroupKey || scored.teamKey || scored.score.ageGroup || '');
    if (currentGame) _adoptLocalDraftForOfficialGame(currentGame, explicitGroupKey || scored.teamKey || scored.score.ageGroup || '');
    const nextScore = {
      ...scored.score,
...
function _getScorerSessions() {
  try { return JSON.parse(localStorage.getItem(STORE.SCORER_SESSIONS) || '{}'); }
  catch { return {}; }
}

function _saveScorerSessions(sessions) {
  localStorage.setItem(STORE.SCORER_SESSIONS, JSON.stringify(sessions || {}));
}

function _getScorerConflicts() {
  try { return JSON.parse(localStorage.getItem(STORE.SCORER_CONFLICTS) || '[]'); }
  catch { return []; }
}

function _saveScorerConflicts(conflicts) {
  const next = Array.isArray(conflicts) ? conflicts.slice(0, 25) : [];
  localStorage.setItem(STORE.SCORER_CONFLICTS, JSON.stringify(next));
  state.scorerConflicts = next;
}

function _recordScorerConflict(type, details = {}) {
  const scopedKey = details.scopedKey || _scopedGameKey(details.gameId || '', details.groupKey || '');
  const next = [
    {
      type,
      scopedKey,
      groupKey: details.groupKey || _contextGroupKey(scopedKey),
      gameId: details.gameId || _gameIdOnly(scopedKey),
      localKey: details.localKey || '',
      officialGameId: details.officialGameId || '',
      opponent: details.opponent || '',
      time: details.time || '',
      detectedAt: new Date().toISOString(),
      note: details.note || '',
    },
    ..._getScorerConflicts().filter(item => !(item.type === type && item.scopedKey === scopedKey && item.localKey === (details.localKey || ''))),
  ];
  _saveScorerConflicts(next);
}
...
function _buildRecoveredDraftsCard() {
  const recovered = state.recoveredScorerSessions || [];
  const conflicts = state.scorerConflicts || [];
  if (!recovered.length && !conflicts.length) return '';
  ...
  const conflictItems = conflicts.slice(0, 3).map(item => {
    const ageLabel = TEAM_OPTIONS.find(t => t.key === (item.groupKey || ''))?.label || item.groupKey || 'Team';
    const title = [ageLabel, item.opponent ? `vs ${normalizeOpponentName(item.opponent)}` : '', item.time || '']
      .filter(Boolean)
      .join(' · ');
    return `<div class="recovered-conflict-item">
      <div class="recovered-conflict-title">${escHtml(title || 'Scorer sync issue')}</div>
      <div class="recovered-conflict-copy">${escHtml(item.note || 'Manual draft was matched to an official game slot.')}</div>
    </div>`;
  }).join('');
  return `<div class="card tab-card recovered-drafts-card">
    <div class="recovered-drafts-head">
      <div>
...
function setFollowedLiveGameId(gameOrRef, explicitGroupKey = '') {
  localStorage.setItem(STORE.LIVE_ACTIVITY_GAME, _gameRef(gameOrRef, explicitGroupKey));
}

function clearFollowedLiveGameId(gameOrRef = null, explicitGroupKey = '') {
  if (!gameOrRef) {
    localStorage.removeItem(STORE.LIVE_ACTIVITY_GAME);
    return;
  }
  const scopedKey = _gameRef(gameOrRef, explicitGroupKey);
  if (getFollowedLiveGameId() === scopedKey) localStorage.removeItem(STORE.LIVE_ACTIVITY_GAME);
}

async function _ensureFollowedLiveActivity(gameId) {
  if (!_isNativePlatform() || window.Capacitor?.getPlatform?.() !== 'ios') return false;
  ...
  const plugin = window.Capacitor?.Plugins?.LiveActivity ||
    (window.Capacitor?.nativePromise
      ...
      : null);
  if (!plugin?.updateActivity) return false;
  const score = getLiveScore(scopedGameId);
  if (!score || !score.gameState || score.gameState === 'pre' || score.gameState === 'final' || score.gameState === 'so_w' || score.gameState === 'so_l') {
    clearFollowedLiveGameId(scopedGameId);
    return false;
  }
...
function _getTournamentIdForGame(gameOrRef, explicitGroupKey = '') {
  const tournament = getTournamentForGroup(_contextGroupKey(gameOrRef, explicitGroupKey)) || TOURNAMENT || {};
  return String(tournament.id || '');
}

function _getScorerSessionKey(gameOrRef, explicitGroupKey = '') {
  const scopedKey = _scopedGameKey(gameOrRef, explicitGroupKey);
  const tournamentId = _getTournamentIdForGame(gameOrRef, explicitGroupKey);
  return tournamentId ? `${tournamentId}::${scopedKey}` : scopedKey;
}

function _getScorerSession(gameOrRef, explicitGroupKey = '') {
  const sessions = _getScorerSessions();
...
function _upsertScorerSession(gameOrRef, explicitGroupKey = '', extra = {}) {
  ...
  sessions[sessionKey] = {
    ...
    location: game?.location || previous.location || score?.location || '',
    team: typeof score?.team === 'number' ? score.team : (previous.team ?? 0),
    opp: typeof score?.opp === 'number' ? score.opp : (previous.opp ?? 0),
    gameState: score?.gameState || previous.gameState || 'pre',
    eventCount: _meaningfulEventCount(score),
    status: extra.status || previous.status || 'open',
...
function _closeScorerSession(gameOrRef, explicitGroupKey = '', status = 'closed') {
  const sessionKey = _getScorerSessionKey(gameOrRef, explicitGroupKey);
  const sessions = _getScorerSessions();
...
function _hasActiveScorerSession(gameOrRef, explicitGroupKey = '') {
  ...
  const score = getLiveScore(gameOrRef);
  const meaningfulEvents = Math.max(Number(session.eventCount || 0), _meaningfulEventCount(score));
  const activeState = score?.gameState || session.gameState || 'pre';
  if (activeState === 'pre' || activeState === 'final') return false;
  return meaningfulEvents > 0 || typeof score?.team === 'number' || typeof score?.opp === 'number';
}
...
function afterScore(gameId) {
  ...
  if (_afterGs && _afterGs.gameState === 'pre' && !_hasMeaningfulLiveScoreData(_afterGs)) {
    ...
    if (window._activeLA?.gameId === gameRef) {
      ...
      clearFollowedLiveGameId(gameRef);
...
function getLiveScore(gameId) {
  ...
  _attachScoreContext(s, gameId, scopedGroup);
  return s;
}
...
function reopenFinalizedGame(gameId) {
  ...
  _setLiveScore(gameId, s);
  _upsertScorerSession(gameId, _contextGroupKey(gameId), { status: 'open' });
  afterScore(gameId);
...
function resetToPreGame(gameId) {
  ...
  s.needsFinalizationAt = null;
  _setLiveScore(gameId, s);
  _closeScorerSession(gameId, _contextGroupKey(gameId), 'reset');
  afterScore(gameId);
...
function _buildScoreDetailScorerPanel(game, s) {
  ...
  const timingRow = `
    <div class="auto-clock-wrap${s.timerExpired ? ' is-expired' : ''}">
      ...
      <div class="auto-clock-controls auto-clock-controls-timeout">
        ${(cs.timeoutLengths||[]).map(m => {
          const used = teamTOUsed.includes(m);
          return `<button class="auto-clock-btn auto-clock-timeout-btn${used ? ' auto-clock-timeout-used' : ''}" ${used ? 'disabled' : ''} onclick="callTeamTimeout('${gid}', ${JSON.stringify(m)})">Team TO ${fmtTOLabel(m)}</button>`;
        }).join('')}
        ${(cs.timeoutLengths||[]).map(m => {
          const used = oppTOUsed.includes(m);
          return `<button class="auto-clock-btn auto-clock-timeout-btn auto-clock-timeout-opp${used ? ' auto-clock-timeout-used' : ''}" ${used ? 'disabled' : ''} onclick="callOppTimeout('${gid}', ${JSON.stringify(m)})">Opp TO ${fmtTOLabel(m)}</button>`;
        }).join('')}
      </div>` : ''}
    </div>`;
  return `
...
function _phaseSeconds(phase, cs) {
  if (phase === 'timeout' || phase === 'opp_timeout') return (cs.timeoutMins || 1) * 60;
...
function _phaseLabel(phase) {
  return { q1:'Q1', break12:'Quarter Break', q2:'Q2', halftime:'Half Time', q3:'Q3', break34:'Quarter Break', q4:'Q4', done:'Final', h1:'H1', h2:'H2', timeout:'Team Timeout', opp_timeout:'Opponent Timeout' }[phase] || phase?.toUpperCase() || '';
}
...
function callTeamTimeout(gameId, lengthMins) {
  ...
  latest.clock = fmtClock(latest.timerSecondsLeft);
  latest.timerRunning = true;
  latest.timerStartedAt = Date.now();
  latest.timerExpired = false;
  latest.timerAdvanceTo = null;
  latest._clockExpiring = false;
  latest.teamTimeoutsUsed = [...latest.teamTimeoutsUsed, lengthMins];
...
function callOppTimeout(gameId, lengthMins) {
  ...
  latest.timerAdvanceTo = null;
  latest._clockExpiring = false;
  latest.oppTimeoutsUsed = [...latest.oppTimeoutsUsed, lengthMins];
  _setLiveScore(gameId, latest);
  saveLiveScores();
  ensureClockTicker();
...
function renderSettingsTab() {
  ...
  const scorerConflicts = (state.scorerConflicts || []).map(item => {
    const ageLabel = TEAM_OPTIONS.find(t => t.key === (item.groupKey || ''))?.label || item.groupKey || 'Team';
    const label = [ageLabel, item.opponent ? `vs ${normalizeOpponentName(item.opponent)}` : '', item.time || '']
      .filter(Boolean)
      .join(' · ');
    return `<div class="settings-item" style="cursor:default">
      ...
      <div class="settings-item-text">
        <div class="settings-item-label">${escHtml(label || 'Scorer sync issue')}</div>
        <div class="settings-item-value">${escHtml(item.note || 'Manual scorer draft was rebound to an official game.')}</div>
      </div>
    </div>`;
  }).join('');
  ...
  el.innerHTML = `
    ...
    ${scorerConflicts ? `
    <div class="settings-section" style="margin-bottom:30px">
      <div class="settings-section-title">Scorer Sync Issues</div>
      ${scorerConflicts}
    </div>` : ''}
...
function getScoreDetailContext() {
  ...
  return _withScoreTournamentContext(detail.groupKey || '', () => {
    const game = _findGameByRef(detail.gameId, detail.groupKey || '');
    if (!game) return null;
    return {
      ...detail,
...
function renderScoresTab() {
  ...
  if (state.scoreDetail && detailCtx) {
    ...
    el.innerHTML = dirHtml + recoveryCard + buildScoreDetailView(detailCtx);
    return;
  }
  ...
  try {
    ...
    for (const remoteEntry of remotePayloads) {
      ...
      const localScorerOwnsGame = _isScorerUnlockedForGame(scopedKey, remoteScore.ageGroup || remoteScore.score?.ageGroup || '')
        && (_hasActiveScorerSession(scopedKey, remoteScore.ageGroup || remoteScore.score?.ageGroup || '') || myGames.has(scopedKey) || myGames.has(gameId))
        && !local._remote
        && !!local.gameState
        && local.gameState !== 'pre';
      ...
      if (scoreData.gameState === 'pre') {
        ...
        if (window._activeLA?.gameId === scopedKey || window._activeLA?.gameId === gameId) {
          ...
          clearFollowedLiveGameId(window._activeLA?.gameId || scopedKey);
          window._activeLA = null;
    ...
    if (_isNativePlatform() && window.Capacitor?.getPlatform?.() === 'ios') {
      const followedGameId = getFollowedLiveGameId();
      if (followedGameId && !window._activeLA) {
        const followedScore = state.liveScores[followedGameId] || state.liveScores[_gameIdOnly(followedGameId)];
        if (followedScore && followedScore.gameState && followedScore.gameState !== 'pre' && followedScore.gameState !== 'final' && followedScore.gameState !== 'so_w' && followedScore.gameState !== 'so_l') {
          const reattached = await _ensureFollowedLiveActivity(followedGameId);
          if (!reattached) toggleLiveActivity(followedGameId).catch(() => {});
        } else if (followedScore && (followedScore.gameState === 'final' || followedScore.gameState === 'so_w' || followedScore.gameState === 'so_l')) {
          clearFollowedLiveGameId(followedGameId);
        }
      }
      ...
      if (window._activeLA) {
        ...
        if (ls) {
          ...
          if (isEnded) {
            ...
            clearFollowedLiveGameId(laGameId);
            window._activeLA = null;
          } else {
            try {
...
async function toggleLiveActivity(gameId) {
  ...
  try { await _syncWidgetsAll(); } catch {}
  const score = getLiveScore(gameId);
  ...
  try {
    ...
    setFollowedLiveGameId(scopedGameId);
